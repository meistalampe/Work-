...
b1 ='F:\GitHubRepositories\Work-\ClosedLoopVirtualRealityfortheTreatmentofPhobias\Matlab\ECG_Analysis\Raw Data Archive\';
b2 = answer{4,1};
...
baselineData = dlmread(signalname,'\t', 3, 0);
% the signal is extracted from the data matrix, channel number needed
signal = data(:,channel);
nSamples = length(data);

baseline = baselineData(:,channel);
bSamples = length(baseline);
...
baseline_adj = (((baseline./2.^10).* 3.3)./0.132);
...
baseline_co = baseline_adj(cSamples:(end-cSamples));
ntime_co = ntime(cSamples:(end-cSamples));
btime_co = btime(cSamples:(end-cSamples));

% plot adjusted and cut signal
figure;
plot(ntime_co,signal_co);
title('EDA Time Domain')
xlabel('Time (t)')
ylabel('X(t)')

figure;
plot(btime_co,baseline_co);
title('Baseline Time Domain')
xlabel('Time (t)')
ylabel('X(t)')
