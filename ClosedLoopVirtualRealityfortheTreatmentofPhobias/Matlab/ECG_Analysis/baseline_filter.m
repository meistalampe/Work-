function [ signal_filt ] = baseline_filter(signal,time,Fs,filepath )
...
figure;
plot(f,P(1:n/2+1)) 
title('Baseline ECG Frequency Domain')
xlabel('Frequency (f)')
ylabel('|P(f)|')

savefig([filepath filesep 'Baseline Frequency Domain']);
saveas(gcf, [filepath filesep 'Baseline Frequency Domain'], 'png');
...
title 'Baseline ECG filtered';
xlabel 'time [s]';
ylabel 'voltage [mV] ';
hold off;

savefig([filepath filesep 'Baseline ECG filtered']);
saveas(gcf, [filepath filesep 'Baseline ECG filtered'], 'png');
