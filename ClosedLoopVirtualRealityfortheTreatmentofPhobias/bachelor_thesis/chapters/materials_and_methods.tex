...
%raw data adjustment
The second step was the raw data adjustment where we converted the sampled data into units of micro Siemens. The values were sampled from channels A1 and A2 of the BITalino device, for EDA and ECG respectively, and 10 bit coded (see \ref{BitImg}).
The adjustment was achieved by applying the transfer functions given in the sensor's data sheets.
...
Once the general preprocessing of the data was finished we continued by applying individual methods to each data set.

\subsubsection{ECG Processing}
%-filter
In regard to the result of the fft analysis we decided to use 
